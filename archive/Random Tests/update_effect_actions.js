...
const filePath = './heic_sim.js';
const content = fs.readFileSync(filePath, 'utf8');

// Find the start and end of EFFECT_ACTIONS
const startMarker = 'const EFFECT_ACTIONS = {';
const endMarker = '  };';

const startIndex = content.indexOf(startMarker);
if (startIndex === -1) {
  console.error('Could not find EFFECT_ACTIONS start marker');
  process.exit(1);
}

// Find the end marker after the start (we need to find the right closing brace)
let endIndex = -1;
let braceCount = 0;
let inEffectActions = false;
...
for (let i = startIndex; i < content.length; i++) {
  ...
  if (char === '{') {
    braceCount++;
    inEffectActions = true;
  } else if (char === '}' && inEffectActions) {
    braceCount--;
    if (braceCount === 0) {
...
if (endIndex === -1) {
  console.error('Could not find EFFECT_ACTIONS end marker');
  process.exit(1);
}

console.log(`Found EFFECT_ACTIONS from index ${startIndex} to ${endIndex}`);
console.log(`Current EFFECT_ACTIONS size: ${endIndex - startIndex} characters`);

// Create the new EFFECT_ACTIONS with all 124 actions
const newEffectActions = `const EFFECT_ACTIONS = {
...
const newContent = content.substring(0, startIndex) + newEffectActions + content.substring(endIndex);

// Write the updated content back to the file
fs.writeFileSync(filePath, newContent, 'utf8');

console.log('✅ Successfully updated EFFECT_ACTIONS in heic_sim.js');
console.log('✅ Now supports all 124 action types from migrated effects');
console.log('✅ New EFFECT_ACTIONS size:', newEffectActions.length, 'characters');